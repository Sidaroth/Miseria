...
*/
	...
	checkSurroundings: function() {
		...
		var tx = Math.floor(this.pos.x / ig.game.layer_fg.tilesize),
			...
			// this.coll_dir.right = c_data[ty][tx+1] || c_data[ty+1][tx+1];
			if (typeof c_data[ty] !== 'undefined' && typeof c_data[ty][tx+1] !== 'undefined' && c_data[ty][tx+1] > 0) {
				this.coll_dir.right = c_data[ty][tx+1];
			} else if (typeof c_data[ty+1] !== 'undefined' && typeof c_data[ty+1][tx+1] !== 'undefined' && c_data[ty+1][tx+1] > 0) {
				this.coll_dir.right = c_data[ty+1][tx+1];
			}

			this.coll_dir.bottom = c_data[ty+2] ? c_data[ty+2][tx] : 0;

			// this.coll_dir.left = c_data[ty][tx-1] || c_data[ty+1][tx-1];
			if (typeof c_data[ty] !== 'undefined' && typeof c_data[ty][tx-1] !== 'undefined' && c_data[ty][tx-1] > 0) {
